...
Class FreeASPUpload
	...
	Public Sub Save(path)
		Dim streamFile, fileItem, filePath, fileDateName, folderX, folderY, LL, Xfs, Xf, CYM
		...
		For Each fileItem In UploadedFiles.Items
			CYM=Year(Now)&Right("0"&Month(Now),2)
			fileDateName = CYM&Right("0"&Day(Now),2)&Right("0"&Hour(Now),2)&Right("0"&Minute(Now),2)&Right("0"&Second(Now),2)&"."&Split(fileItem.FileName&".",".")(UBound(Split(fileItem.FileName&".","."))-1)
			filePath = path & CYM & "\" & fileDateName
			
			folderX=Split(filePath,"\")
			set Xfs=Server.CreateObject("Scripting.FileSystemObject")
			folderY=folderX(0)&"\"&folderX(1)&"\"
			For LL=2 to UBound(folderX)-1
				folderY=folderY&folderX(LL)&"\"
			if Xfs.FolderExists(folderY)=false then
			set Xf=Xfs.CreateFolder(folderY)
			set Xf=nothing
			end if
			Next
			set Xfs=nothing
			...
			fileItem.FileName = fileDateName
		 Next
	End Sub
